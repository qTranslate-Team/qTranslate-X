...
{//action/filters
	...
	function qtranxf_slug_detect_language( $lang, &$urlinfo ) {
		...
		if ( $urlinfo['wp-path'] != $urlinfo['wp-paths'][ $lang ] ) {
			if ( empty( $urlinfo['doredirect'] ) ) {
				$urlinfo['doredirect'] = 'slug';
			}
		}
...
{//utilities
	...
	function qtranxf_slug_get_translations( $name ) {
		...
		if ( isset( $q_config['slugs-cache']['names'][ $name ] ) ) {
			return $q_config['slugs-cache']['names'][ $name ];
		}
	...
	function qtranxf_slug_translation( $slug ) {
		global $q_config, $wpdb;
		if ( isset( $q_config['slugs-cache']['slugs'][ $slug ] ) ) {
			return $q_config['slugs-cache']['slugs'][ $slug ];
		}
