...
{// configuration
	...
	function qtranxf_slug_url_mode_choices( $permalink_is_query, $url_mode ) {
        ...
        <label title="Slug Mode"><input type="radio" name="url_mode" value="<?php echo QTX_URL_SLUG; ?>" <?php checked( $url_mode, QTX_URL_SLUG );
			disabled( $permalink_is_query ) ?> /> <?php echo __( 'Use Slug Mode.', 'qtranslate' ) . ' ' . __( 'SEO friendly.', 'qtranslate' ) ?></label>
	...
	function qtranxf_slug_config( $request_uri ) {
        ...
        <table class="form-table" id="qtranxf_slug_config">
			<?php if ( $nopermalinks ) { ?>
                <tr>
                    <th scope="row"
                        colspan="2"><?php printf( __( 'You have to use pretty %spermalinks%s in order to take advantage of multilingual slugs.', 'qtranslate' ), '<a href="' . admin_url( 'options-permalink.php' ) . '">', '</a>' ) ?></th>
                </tr>
			<?php } else { ?>
                <tr>
				...
				if ( ! empty( $q_config['slugs'] ) ) {
					...
					if ( ! empty( $objects ) ) {
                        ...
                        <tr>
                            ...
                            <td><p class="qtranxs_explanation"><?php echo __( 'Multilingual slug base for custom taxonomies.', 'qtranslate' ) . ' ';
									printf( __( 'The default language value is as provided in the argument %s of a call to function %s made by a 3rd-party, which created the term.', 'qtranslate' ), '<code>$args[\'rewrite\'][\'slug\']</code>', '<a href="http://codex.wordpress.org/Function_Reference/register_taxonomy" target="_blank"><code>register_taxonomy</code></a>' ) ?></p>
                            </td>
                        </tr>
						<?php
					...
					if ( ! empty( $objects ) ) {
                        ...
                        <tr>
                            ...
                            <td>
                                <p class="qtranxs_explanation"><?php printf( __( 'Multilingual slug base for WordPress built-in post types. The default value is defined by %sWordPress%s.', 'qtranslate' ), '<a href="https://codex.wordpress.org/Post_Types" target="_blank">', '</a>' ) ?></p>
                            </td>
                        </tr>
						<?php
					...
					if ( ! empty( $objects ) ) {
                        ...
                        <tr>
                            ...
                            <td><p class="qtranxs_explanation"><?php echo __( 'Multilingual slug base for custom post types.', 'qtranslate' ) . ' ';
									printf( __( 'The default language value is as provided in the argument %s of a call to function %s made by a 3rd-party, which created the term.', 'qtranslate' ), '<code>$args[\'rewrite\'][\'slug\']</code>', '<a href="http://codex.wordpress.org/Function_Reference/register_post_type" target="_blank"><code>register_post_type</code></a>' ) ?></p>
                            </td>
                        </tr>
						<?php
					...
					?>
                    <tr>
                        <td colspan="2" id="qtranxf_slug_lsb_bottom"></td>
                    </tr>
					<?php
				}//if(!empty($q_config['slugs']))
			...
			if ( ! empty( $q_config['slugs_opt']['mv']['terms'][ $group ]['values'][ $key ] ) ) {
				...
				if ( $slug == $value_new ) {
					...
					if ( empty( $q_config['slugs_opt']['mv']['terms'][ $group ]['values'] ) ) {
						unset( $q_config['slugs_opt']['mv']['terms'][ $group ] );
					}
					if ( empty( $q_config['slugs_opt']['mv']['terms'] ) ) {
						unset( $q_config['slugs_opt']['mv']['terms'] );
					}
					if ( empty( $q_config['slugs_opt']['mv'] ) ) {
						unset( $q_config['slugs_opt']['mv'] );
					}
					if ( empty( $q_config['slugs_opt'] ) ) {
						delete_option( 'qtranslate_slugs_opt' );
					} else {
