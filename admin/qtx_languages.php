...
function qtranxf_langs_preset() {
	...
	if ( $q_config['use_strftime'] == QTX_DATE_WP ) {
		foreach ( $langs as $lang => $props ) {
			if ( isset( $props['date_format'] ) ) {
				$langs[ $lang ]['date_format'] = qtranxf_convert_strftime2date( $props['date_format'] );
			}
			if ( isset( $props['time_format'] ) ) {
				$langs[ $lang ]['time_format'] = qtranxf_convert_strftime2date( $props['time_format'] );
			}
		}
	}
