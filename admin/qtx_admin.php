...
function qtranxf_add_admin_head_js( $enqueue_script = true ) {
	if ( $enqueue_script ) {
		...
		readfile( $plugin_dir_path . 'js/options.js' );
...
function qtranxf_add_admin_highlight_css() {
	...
	switch ( $highlight_mode ) {
		case QTX_HIGHLIGHT_MODE_CUSTOM_CSS:
			...
			break;
		default:
			echo qtranxf_get_admin_highlight_css( $highlight_mode );
	}
	echo '</style>' . PHP_EOL;
}
	...
	*/
    ...
    <div id="qtranxs-langsw" class="qtranxslangswdiv">
		...
		<span class="add-to-menu">
			<input type="submit"<?php disabled( $nav_menu_selected_id, 0 ) ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu', 'qtranslate' ) ?>" name="add-qtranxs-langsw-menu-item"
