			...
			foreach ( $services as $service_id => $service ) {
				...
				foreach ( $service['service_required_fields'] as $field ) {
					if ( isset( $_POST[ 'qts_' . $service_id . '_' . $field['name'] ] ) ) {
						...
						if ( $_POST[ 'qts_' . $service_id . '_' . $field['name'] ] == '' && $field['name'] == 'password' ) {
							continue;
						}
    ...
    <ul>
		...
		foreach ( $languages as $language ) {
			...
			if ( isset( $_REQUEST['post'] ) ) {
				?>
                <li><img src="<?php echo qtranxf_flag_location() . $q_config['flag'][ $language ]; ?>" alt="<?php echo $q_config['language_name'][ $language ]; ?>"> <a
                            href="edit.php?page=qtranslate_services&post=<?php echo intval( $_REQUEST['post'] ) ?>&target_language=<?php echo $language; ?>"><?php echo $q_config['language_name'][ $language ]; ?> <span
                                class="qsprice"></span></a></li>
				<?php
			} else {
...
function qts_config_hook( $request_uri ) {
    ...
    <table class="form-table">
        <tr>
            ...
            <td>
				<?php if ( $q_config['qtranslate_services'] && ! qts_isOpenSSLAvailable() ) {
                ...
                <label for="qtranslate_services"><input type="checkbox" name="qtranslate_services" id="qtranslate_services"
                                                        value="1"<?php checked( $q_config['qtranslate_services'] ) ?>/> <?php _e( 'Enable qTranslate Services', 'qtranslate' ) ?></label>
                <br/>
					...
					<?php if ( is_array( $orders ) && sizeof( $orders ) > 0 ) { ?>
                        <table class="widefat">
							...
							foreach ( $orders as $order ) {
								...
								?>
                                <tr>
                                    <td class="qts_no-bottom-border"><a href="post.php?action=edit&post=<?php echo $order['post_id']; ?>"
                                                                        title="<?php printf( __( 'Edit %s', 'qtranslate' ), $post->post_title ) ?>"><?php echo $post->post_title; ?></a></td>
                                    <td class="qts_no-bottom-border"><?php if ( isset( $services[ $order['service_id'] ] ) ): ?><a href="<?php echo $services[ $order['service_id'] ]['service_url']; ?>"
                                                                                                                                   title="<?php _e( 'Website', 'qtranslate' ) ?>"><?php echo $services[ $order['service_id'] ]['service_name']; ?></a><?php endif; ?>
                                    </td>
                            ...
                            <ul>
								...
								foreach ( $services as $service_id => $service ) {
									...
									if ( ! $requirements_matched ) {
										?>
                                        <li>
                                            <label><input type="radio" name="service_id" disabled="disabled"/> <b><?php echo qtranxf_useCurrentLanguageIfNotFoundUseDefaultLanguage( $service['service_name'] ) ?></b> ( <a
                                                        href="<?php echo $service['service_url']; ?>" target="_blank"><?php _e( 'Website', 'qtranslate' ) ?></a> )</label>
										...
										?>
                                        <li><label><input type="radio" id="qts_service_<?php echo $service['service_id']; ?>" onclick="chooseservice(this.value)"
                                                          value="<?php echo $service['service_id']; ?>"<?php checked( $service['service_id'], $default_service ) ?> <?php echo $confirm ? 'disabled="disabled"' : 'name="service_id"'; ?> />
                                                <b><?php echo qtranxf_useCurrentLanguageIfNotFoundUseDefaultLanguage( $service['service_name'] ) ?></b> ( <a href="<?php echo $service['service_url']; ?>"
                                                                                                                                                             target="_blank"><?php _e( 'Website', 'qtranslate' ) ?></a> )</label>
                                            <p class="service_description"><?php echo qtranxf_useCurrentLanguageIfNotFoundUseDefaultLanguage( $service['service_description'] ) ?></p></li>
										<?php
									}
								}
								?>
